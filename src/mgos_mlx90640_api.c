...
/*
 * The following is the Melexis API with some modifications to incorporate
 * into the Mongoose OS framework. These changes were made
 * by Paul Austen (pausten.os@gmail.com) on 23/1/2019.
 */
...
int MLX90640_InterpolateOutliers(uint16_t *frameData, uint16_t *eepromData) {
	for (int x = 0; x < MLX90640_FRAME_BUFFER_SIZE; x++) {
        ...
        if (broken | outlier){
			...
			if (x + 33 < MLX90640_FRAME_BUFFER_SIZE) {
				val += frameData[x + 33];
				val += frameData[x + 31];
				count += 2;
			} else if (x + 31 < MLX90640_FRAME_BUFFER_SIZE) {
				val += frameData[x + 31];
				count += 1;
			}
...
int MLX90640_GetFrameData(uint8_t slaveAddr, uint16_t *frameData) {
	...
	while (dataReady != 0 && cnt < 5) {
		...
		error = MLX90640_I2CRead(slaveAddr, 0x0400, 832, frameData);
		if (error != 0) {
			LOG(LL_INFO, ("%s: frameData read error", __FUNCTION__));
			return error;
		}
	...
	if (cnt > 4) {
		LOG(LL_INFO, ("%s: cnt > 4 error", __FUNCTION__));
		return -8;
	}
...
void MLX90640_CalculateTo(uint16_t *frameData, const paramsMLX90640 *params,
	...
	float irDataCP[2];
	float irData = 0;
	float alphaCompensated = 0;
	uint8_t mode = 0;
	int8_t ilPattern = 0;
	int8_t chessPattern = 0;
	int8_t pattern = 0;
	int8_t conversionPattern = 0;
	float Sx = 0;
	float To = 0;
	float alphaCorrR[4];
	int8_t range = 0;
	...
	alphaCorrR[3] = alphaCorrR[2]
			* (1 + params->ksTo[3] * (params->ct[3] - params->ct[2]));
...
//------------------------- To calculation -------------------------------------
	...
	for (int pixelNumber = 0; pixelNumber < MLX90640_FRAME_BUFFER_SIZE; pixelNumber++) {
...
float MLX90640_GetVdd(uint16_t *frameData, const paramsMLX90640 *params) {
	...
	resolutionCorrection = pow(2, (double) params->resolutionEE)
...
float MLX90640_GetTa(uint16_t *frameData, const paramsMLX90640 *params) {
	...
	ptatArt = frameData[MLX90640_FRAME_BUFFER_SIZE];
	if (ptatArt > 32767) {
...
static void ExtractAlphaParameters(uint16_t *eeData, paramsMLX90640 *mlx90640) {
	...
	uint8_t accRowScale = 0;
	uint8_t accColumnScale = 0;
	uint8_t accRemScale = 0;
...
static void ExtractOffsetParameters(uint16_t *eeData, paramsMLX90640 *mlx90640) {
	...
	int p = 0;
	int16_t offsetRef = 0;
	uint8_t occRowScale = 0;
	uint8_t occColumnScale = 0;
	uint8_t occRemScale = 0;
...
static int ExtractDeviatingPixels(uint16_t *eeData, paramsMLX90640 *mlx90640) {
	...
	pixCnt = 0;
	while (pixCnt < MLX90640_FRAME_BUFFER_SIZE && brokenPixCnt < 5 && outlierPixCnt < 5) {
